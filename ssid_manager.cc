...
void SsidManager::LoadFromNvs() {
  ...
  for (int i = 0; i < MAX_WIFI_SSID_COUNT; i++) {
    ...
    if (nvs_get_str(nvs_handle, password_key.c_str(), password, &length) !=
        ESP_OK) {
      continue;
    }
...
void SsidManager::SaveToNvs() {
  ...
  for (int i = 0; i < MAX_WIFI_SSID_COUNT; i++) {
    ...
    if (i < ssid_list_.size()) {
      ...
      nvs_set_str(nvs_handle, password_key.c_str(),
...
void SsidManager::AddSsid(const std::string &ssid,
  ...
  for (auto &item : ssid_list_) {
    ESP_LOGI(TAG, "compare [%s:%d] [%s:%d]", item.ssid.c_str(),
             item.ssid.size(), ssid.c_str(), ssid.size());
    if (item.ssid == ssid) {
