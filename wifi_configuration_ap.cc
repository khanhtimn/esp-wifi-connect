...
extern const char
    index_html_start[] asm("_binary_wifi_configuration_html_start");
extern const char
    done_html_start[] asm("_binary_wifi_configuration_done_html_start");
...
void WifiConfigurationAp::Start() {
  ...
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      WIFI_EVENT, ESP_EVENT_ANY_ID, &WifiConfigurationAp::WifiEventHandler,
      this, &instance_any_id_));
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      IP_EVENT, IP_EVENT_STA_GOT_IP, &WifiConfigurationAp::IpEventHandler, this,
      &instance_got_ip_));
...
#endif
  char ssid[32];
  snprintf(ssid, sizeof(ssid), "%s-%02X%02X", ssid_prefix_.c_str(), mac[4],
           mac[5]);
...
void WifiConfigurationAp::StartWebServer() {
  ...
  httpd_uri_t form_submit = {
      ...
      .handler = [](httpd_req_t *req) -> esp_err_t {
        ...
        if (!cJSON_IsString(ssid_item) || (ssid_item->valuestring == NULL) ||
            (strlen(ssid_item->valuestring) >= 33)) {
          cJSON_Delete(json);
          httpd_resp_send(req, "{\"success\":false,\"error\":\"Invalid SSID\"}",
        ...
        if (!this_->ConnectToWifi(ssid_str, password_str)) {
          cJSON_Delete(json);
          httpd_resp_send(req,
                          "{\"success\":false,\"error\":\"Failed to connect to "
                          "the Access Point\"}",
                          HTTPD_RESP_USE_STRLEN);
          return ESP_OK;
        }
  ...
  httpd_uri_t done_html = {.uri = "/done.html",
                           ...
                           .handler = [](httpd_req_t *req) -> esp_err_t {
                             httpd_resp_set_hdr(req, "Connection", "close");
                             httpd_resp_send(req, done_html_start,
                                             strlen(done_html_start));
                             return ESP_OK;
                           },
                           .user_ctx = NULL};
  ...
  for (const auto &url : captive_portal_urls) {
    httpd_uri_t redirect_uri = {.uri = url,
                                .method = HTTP_GET,
  ...
  httpd_uri_t advanced_submit = {
      ...
      .handler = [](httpd_req_t *req) -> esp_err_t {
        ...
        if (cJSON_IsBool(remember_bssid)) {
          ...
          err =
              nvs_set_u8(nvs, "remember_bssid", this_->remember_bssid_ ? 1 : 0);
          if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG,
                 "Saved settings: ota_url=%s, max_tx_power=%d, "
                 "remember_bssid=%d, sleep_mode=%d",
                 this_->ota_url_.c_str(), this_->max_tx_power_,
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid,
  ...
  EventBits_t bits = xEventGroupWaitBits(
      event_group_, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT, pdTRUE, pdFALSE,
#ifdef CONFIG_SOC_WIFI_SUPPORT_5G
      pdMS_TO_TICKS(25000)
#else
      pdMS_TO_TICKS(10000)
...
void WifiConfigurationAp::IpEventHandler(void *arg, esp_event_base_t event_base,
                                         int32_t event_id, void *event_data) {
  WifiConfigurationAp *self = static_cast<WifiConfigurationAp *>(arg);
...
void WifiConfigurationAp::StartSmartConfig() {
  // 注册SmartConfig事件处理器
  ESP_ERROR_CHECK(esp_event_handler_instance_register(
      SC_EVENT, ESP_EVENT_ANY_ID, &WifiConfigurationAp::SmartConfigEventHandler,
...
void WifiConfigurationAp::SmartConfigEventHandler(void *arg,
                                                  esp_event_base_t event_base,
                                                  int32_t event_id,
                                                  void *event_data) {
  WifiConfigurationAp *self = static_cast<WifiConfigurationAp *>(arg);
...
void WifiConfigurationAp::Stop() {
  ...
  // Bỏ thay đổi mode Wi-Fi ở đây để giữ nguyên APSTA cho tới khi provisioning
  // END xử lý. Bỏ destroy ap_netif_ ở đây để tránh làm mất handler khi
  // provisioning manager vẫn còn reference.
  ...
  // {
  //     esp_event_handler_instance_unregister(WIFI_EVENT, ESP_EVENT_ANY_ID,
  //     instance_any_id_); instance_any_id_ = nullptr;
  // }
  // if (instance_got_ip_)
  // {
  //     esp_event_handler_instance_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP,
  //     instance_got_ip_); instance_got_ip_ = nullptr;
  // }
