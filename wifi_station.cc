...
void WifiStation::Stop() {
  ...
  // Clear event group bits to prevent WaitForConnected from returning
  // prematurely on restart
...
void WifiStation::HandleScanResult() {
  ...
  for (int i = 0; i < ap_num; i++) {
    ...
    auto it = std::find_if(
        ssid_list.begin(), ssid_list.end(), [ap_record](const SsidItem &item) {
          return strcmp((char *)ap_record.ssid, item.ssid.c_str()) == 0;
        });
    if (it != ssid_list.end()) {
      ESP_LOGI(TAG,
               "Found AP: %s, BSSID: %02x:%02x:%02x:%02x:%02x:%02x, RSSI: %d, "
               "Channel: %d, Authmode: %d",
               (char *)ap_record.ssid, ap_record.bssid[0], ap_record.bssid[1],
               ap_record.bssid[2], ap_record.bssid[3], ap_record.bssid[4],
               ap_record.bssid[5], ap_record.rssi, ap_record.primary,
               ap_record.authmode);
      WifiApRecord record = {.ssid = it->ssid,
...
void WifiStation::WifiEventHandler(void *arg, esp_event_base_t event_base,
  ...
  if (event_id == WIFI_EVENT_STA_START) {
    ...
    if (this_->reconnect_count_ < MAX_RECONNECT_COUNT) {
      ...
      ESP_LOGI(TAG, "Reconnecting %s (attempt %d / %d)", this_->ssid_.c_str(),
